...
<main>
    <section class="container py-4">
        ...
        <div class="col-lg-6">
            <div class="p-3 default-cursor">
                ...
                <p class="mb-1 mt-1"><span class="text-muted"><small><?= label('inclusive_of_all_taxes', '(Inclusive of all taxes)') ?></small></span></p>
            </div>
            <div>
                <?php
            ...
            <div class="row py-3 px-3 product-add-section align-self-center">
                    ...
                    <form class="mt-2" id="validate-zipcode-form" method="POST">
                        <div class="d-flex flex-wrap gap-2 form-row">
                            <div class=" col-md-6">
                                ...
                                <input type="text" class="form-control" id="zipcode" placeholder="<?= !empty($this->lang->line('zipcode')) ? $this->lang->line('zipcode') : 'Zipcode' ?>" name="zipcode" autocomplete="off" required value="<?= $product['product'][0]['zipcode']; ?>">
                            </div>
                            <button type="submit" class="btn btn-sm btn-primary" id="validate_zipcode"><?= label('check_availability', 'Check Availability') ?></button>
                        </div>
                ...
                if (isset($settings['city_wise_deliverability']) && $settings['city_wise_deliverability'] == 1 && $shiprocket_settings['shiprocket_shipping_method'] != 1) { ?>
                    <form class="mt-2" id="validate-city-form" method="POST">
                        <div class="d-flex flex-wrap gap-2 form-row">
                            ...
                            <button type="submit" class="btn btn-sm btn-primary" id="validate_city"><?= label('check_availability', 'Check Availability') ?></button>
                        </div>
                        <div class="mt-2" id="error_box">
                            <?php if (!empty($product['product'][0]['zipcode'])) { ?>
                                <p class="m-0 text-<?= ($product['product'][0]['is_deliverable']) ? "success" : "danger" ?>">Product is <?= ($product['product'][0]['is_deliverable']) ? "" : "not" ?> delivarable on &quot; <?= $product['product'][0]['zipcode']; ?> &quot; </p>
                ...
                <div class="input-group plus-minus-input mb-3 num-block">
                    ...
                    <div class="product-quantity product-sm-quantity border-0 m-0">
                        <input class="input-group-field input-field-cart-modal in-num" type="number" name="qty" value="<?= (isset($product['product'][0]['minimum_order_quantity']) && !empty($product['product'][0]['minimum_order_quantity'])) ? $product['product'][0]['minimum_order_quantity'] : 1 ?>" data-step="<?= (isset($product['product'][0]['minimum_order_quantity']) && !empty($product['product'][0]['quantity_step_size'])) ? $product['product'][0]['quantity_step_size'] : 1 ?>" data-min="<?= (isset($product['product'][0]['minimum_order_quantity']) && !empty($product['product'][0]['minimum_order_quantity'])) ? $product['product'][0]['minimum_order_quantity'] : 1 ?>" data-max="<?= (isset($product['product'][0]['total_allowed_quantity']) && !empty($product['product'][0]['total_allowed_quantity'])) ? $product['product'][0]['total_allowed_quantity'] : '1' ?>" data-price="<?= $product['product'][0]['min_max_price']['special_price'] ?>" data-id="<?= $product['product'][0]['id'] ?>">
                    </div>
                    <div class="input-group-button">
                        <button type="button" class="button hollow circle plus-btn plus" data-quantity="plus" data-field="quantity" data-max="<?= (isset($product['product'][0]['total_allowed_quantity']) && !empty($product['product'][0]['total_allowed_quantity'])) ? $product['product'][0]['total_allowed_quantity'] : '1' ?> " data-step="<?= (isset($product['product'][0]['minimum_order_quantity']) && !empty($product['product'][0]['quantity_step_size'])) ? $product['product'][0]['quantity_step_size'] : 1 ?>">
                            <i class="fa-solid fa-plus"></i>
                ...
                <div class="d-flex gap-2">
                    <?php if ($product['product'][0]['variants'][0]['cart_count'] != 0) { ?>
                        <a class="buttons btn-6-6 extra-small m-0"><button type="submit" title="Add in Cart" class="col-md-12 btn add-btn"><?= label('add_in_cart', 'Add in Cart') ?></button></a>
                    <?php } else { ?>
                        <a href="" class="">
    ...
    <section class="container">
        ...
        <div class="product-reviews-display px-2">
            <div class="mb-3">
                <h4 class="element-title fw-semibold m-0"><span id="no_ratings"><?= $product['product'][0]['no_of_ratings'] ?> <?= label('review_of_product', 'Review Of Product') ?></span> <?= ucfirst($product['product'][0]['name']) ?></h4>
            </div>
                                ...
                                <a href="<?= base_url('products/details/' . $row['slug']) ?>" class="text-reset text-decoration-none">
                                    <div class="card card-h-418 product-card" data-product-id="<?= $row['id'] ?>">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <a href="#" class="btn add-in-cart-btn w-100 add_to_cart px-1 btn-sm <?= $class_modal ?>" data-product-id="<?= $row['id'] ?>" data-product-variant-id="<?= $variant_id ?>" data-product-title="<?= $row['name'] ?>" data-product-image="<?= $row['image'] ?>" data-product-price="<?= $variant_price; ?>" data-min="<?= $data_min; ?>" data-step="<?= $data_step; ?>" data-product-description="<?= $row['short_description']; ?>" data-bs-toggle="modal" data-bs-target="<?= $modal ?>">
                                                <span class="add-in-cart"><?= label('add_to_cart', 'Add to Cart') ?></span><span class="add-in-cart-icon"><i class="fa-solid fa-cart-shopping
    ...
    <!-- <div class="d-md-none row mobile-sticky-btn w-100 m-0 ">
        <?php if ($product['product'][0]['variants'][0]['cart_count'] != 0) { ?>
            <a class="buttons btn-6-6 extra-small m-0"><button type="submit" title="Add in Cart" class="col-lg-3 col-md-12 btn add-btn"><?= label('add_in_cart', 'Add in Cart') ?></button></a>
        <?php } else { ?>
