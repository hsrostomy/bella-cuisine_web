...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL, $is_detailed_data = 0)
{
    ...
    $t->db->select($discount_filter_data . 'count(p.id) as sales, p.stock_type, p.is_prices_inclusive_tax, p.type ,GROUP_CONCAT(DISTINCT(pa.attribute_value_ids)) as attr_value_ids,p.id,p.stock,p.name,p.category_id,p.short_description,p.slug,p.description,p.is_on_sale,p.sale_discount,p.sale_start_date,p.sale_end_date,p.brand,p.total_allowed_quantity,p.deliverable_type,p.is_attachment_required,p.product_identity,p.deliverable_zipcodes,p.deliverable_cities,p.deliverable_city_type,p.minimum_order_quantity,p.quantity_step_size,p.cod_allowed,p.row_order,p.rating,p.no_of_ratings,p.download_allowed,p.download_type,p.download_link,p.image,p.is_returnable,p.is_cancelable,p.cancelable_till,p.indicator,p.other_images, p.video_type, p.video, p.tags,p.sku, p.warranty_period, p.guarantee_period, p.made_in, b.slug as brand_slug,p.availability,c.name as category_name,tax.percentage as tax_percentage,tax.id as tax_id ')->join(" categories c", "p.category_id=c.id ", 'LEFT')
    ...
    if (!empty($product)) {
        ...
        for ($i = 0; $i < count($product); $i++) {
            ...
            if ($product[$i]['deliverable_city_type'] != NONE && $product[$i]['deliverable_city_type'] != ALL) {
                ...
                // $product[$i]['deliverable_cities'] = isset($product[$i]['deliverable_cities']) && !empty($product[$i]['deliverable_cities']) ? output_escaping(implode(",", $cities)) : '';
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        for ($k = 0; $k < count($order_item_data); $k++) {
            ...
            if (!empty($order_item_data)) {
                ...
                // print_r("price:" .$price);
                // print_r("price2:" .$order_item_data[$k]['tax_percent']);
...
function get_delivery_charge($address_id, $total = 0)
{
    ...
    $delivery_charge = $system_settings['delivery_charge'];
    if ((isset($system_settings['area_wise_delivery_charge']) && !empty($system_settings['area_wise_delivery_charge']))) {
        if (isset($system_settings['pincode_wise_deliverability']) == 1) {
        ...
        if (isset($system_settings['city_wise_deliverability']) == 1) {
            // print_r("here");
...
// function check_cart_products_delivarable($user_id, $area_id = 0, $zipcode = "", $zipcode_id = "", $city_id = "")
...
function orders_count($status = "")
{
    ...
    // if ($t->ion_auth->is_delivery_boy()) {
    //     $user_id = $t->session->userdata('user_id');
    //     $t->db->where('delivery_boy_id', $user_id);
    // }
