...
class Customer_model extends CI_Model
{
    ...
    public function get_customer_list()
    {
        ...
        foreach ($cat_search_res as $row) {
            ...
            <a href="' . base_url('admin/orders?user_id=' . $row['id']) . '" class="dropdown-item" title="View Orders" ><i class="fa fa-eye"></i> View Orders</a>

            <a  href="' . base_url('admin/transaction/view-transaction?user_id=' . $row['id']) . '" class="dropdown-item" title="View Transactions"  ><i class="fa fa-money-bill-wave"></i> View Transactions</a>

            <a href="javascript:void(0)" class="view_address  dropdown-item" title="View Address" data-id="' . $row['id'] . '"  data-toggle="modal" data-target="#customer-address-modal" ><i class="far fa-address-book"></i> View Address</a></div>';

            if ($row['active'] == '1') {
                $tempRow['status'] = '<a class="badge bg-success text-white" ></a>';
                $tempRow['status'] .= '<a class="form-switch update_active_status " data-table="users" title="Deactivate" href="javascript:void(0)" data-id="' . $row['id'] . '" data-status="' . $row['active'] . '" ><input class="form-check-input " type="checkbox" role="switch" checked></a>';
            } else {
                $tempRow['status'] = '<a class="badge bg-danger text-white" ></a>';
                $tempRow['status'] .= '<a class="form-switch update_active_status mr-1 mb-1" data-table="users" title="Deactivate" href="javascript:void(0)" data-id="' . $row['id'] . '" data-status="' . $row['active'] . '" ><input class="form-check-input " type="checkbox" role="switch" ></a>';
            }

            // $operate = '<a href="' . base_url('admin/orders?user_id=' . $row['id']) . '" class="btn action-btn btn-primary btn-xs mr-1 mb-1" title="View Orders" ><i class="fa fa-eye"></i></a>';
            // $operate .= '<a  href="' . base_url('admin/transaction/view-transaction?user_id=' . $row['id']) . '" class="btn action-btn btn-danger btn-xs mr-1 mb-1" title="View Transactions"  ><i class="fa fa-money-bill-wave"></i></a>';
            // $operate .= ' <a href="javascript:void(0)" class="view_address  btn action-btn btn-warning btn-xs mr-1 mb-1" title="View Address" data-id="' . $row['id'] . '"  data-toggle="modal" data-target="#customer-address-modal" ><i class="far fa-address-book"></i></a>';
            // if ($row['active'] == '1') {
            //     $tempRow['status'] = '<a class="badge bg-success text-white" >Active</a>';
            //     $operate .= '<a class="btn action-btn btn-success btn-xs update_active_status mr-1" data-table="users" title="Deactivate" href="javascript:void(0)" data-id="' . $row['id'] . '" data-status="' . $row['active'] . '" ><i class="fa fa-toggle-on"></i></a>';
            // } else {
            //     $tempRow['status'] = '<a class="badge bg-danger text-white" >Inactive</a>';
            //     $operate .= '<a class="btn action-btn btn-secondary mr-1 btn-xs update_active_status" data-table="users" href="javascript:void(0)" title="Active" data-id="' . $row['id'] . '" data-status="' . $row['active'] . '" ><i class="fa fa-toggle-off"></i></a>';
            // }
            $tempRow['id'] = $row['id'];
            ...
            if (empty($row['image']) || file_exists(FCPATH . USER_IMG_PATH . $row['image']) == FALSE) {
                $row['image'] = base_url() . NO_USER_IMAGE;
            } else {
                $row['image'] = base_url() . USER_IMG_PATH .  $row['image'];
            }
            $tempRow['image'] = "<div class='image-box-100'><a href='" . $row['image'] . "' data-toggle='lightbox' data-gallery='gallery'> <img src='" . $row['image'] . "' class='h-25' ></a></div>";
