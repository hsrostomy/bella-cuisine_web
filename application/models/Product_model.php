...
// die;
        ...
        if(isset($_GET['status']) && $_GET['status'] != ''){
            ...
            if ($status == '1') {
                ...
                $search_res->where($where);
                // $count_res->where('p.stock <=', $low_stock_limit);
                // $count_res->where('p.status =', $status);
                // $count_res->where('p.availability  =', '1');
                // $count_res->or_where('product_variants.stock <=', $low_stock_limit);
                // $count_res->where('product_variants.availability  =', '1');
                $search_res->group_End();
...
// echo $this -> db->last_query();
    ...
    function get_brand_list($search = "", $offset = 0, $limit = 25)
    {
        ...
        // Add a condition where status is 1
        $where['status'] = 1;
        ...
        if (!empty($brands)) {
            foreach ($brands as &$brand) {
                $brand = output_escaping($brand);
                $brand['relative_path'] = isset($brand['image']) && !empty($brand['image']) ? $brand['image'] : [];
                $brand['image'] = base_url() . $brand['image'];
            }
        }
