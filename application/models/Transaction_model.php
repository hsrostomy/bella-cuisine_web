...
class Transaction_model extends CI_Model
{
    ...
    function get_transactions_list($user_id = '')
    {
        ...
        if (isset($_GET['search']) and $_GET['search'] != '') {
            $search = $_GET['search'];
            $multipleWhere = ['`transactions.id`' => $search, '`transactions.amount`' => $search, '`transactions.date_created`' => $search, '`transactions.order_id`' => $search, 'users.username' => $search, 'users.mobile' => $search, 'users.email' => $search, 'transactions.status' => $search];
        }
        ...
        $count_res = $this->db->select(' COUNT(transactions.id) as `total` ')->join('users', ' transactions.user_id = users.id', 'left');
        ...
        foreach ($txn_search_res as $row) {
            ...
            if ($row['type'] == 'bank_transfer') {
                ...
                <a href="javascript:void(0)" class="edit_transaction dropdown-item" title="Edit" data-id="' . $row['id'] . '" data-txn_id="' . $row['txn_id'] . '" data-status="' . $row['status'] . '" data-message="' . $row['message'] . '"  data-target="#transaction_modal" data-toggle="modal"><i class="fa fa-pen"></i> Edit</a></div>';


                // $operate = ' <a href="javascript:void(0)" class="edit_transaction btn btn-success btn-xs mr-1 mb-1" title="Edit" data-id="' . $row['id'] . '" data-txn_id="' . $row['txn_id'] . '" data-status="' . $row['status'] . '" data-message="' . $row['message'] . '"  data-target="#transaction_modal" data-toggle="modal"><i class="fa fa-pen"></i></a>';
            } else {
                $operate = "";
            }
            $tempRow['id'] = $row['id'];
