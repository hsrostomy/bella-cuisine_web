...
class Delivery_boy_model extends CI_Model
{
    ...
    function get_delivery_boys_list()
    {
        ...
        foreach ($offer_search_res as $row) {
            ...
            <a href="javascript:void(0)" class="edit_btn dropdown-item" title="Edit" data-id="' . $row['id'] . '" data-url="admin/delivery_boys/"><i class="fa fa-pen"></i> Edit</a>

            <a href="javascript:void(0)" class=" fund_transfer dropdown-item" title="Fund Transfer" data-target="#fund_transfer_delivery_boy"   data-toggle="modal" data-id="' . $row['id'] . '" ><i class="fa fa-arrow-alt-circle-right"></i> Fund Transfer</a>


              <a href="javascript:void(0)" class=" dropdown-item" id="delete-delivery-boys" data-id=' . $row['id'] . ' title="Delete" ><i class="fa fa-trash"></i> Delete</a></div>';

            // $operate = '<a href="javascript:void(0)" class="edit_btn action-btn btn btn-primary btn-xs mr-1 mb-1" title="Edit" data-id="' . $row['id'] . '" data-url="admin/delivery_boys/"><i class="fa fa-pen"></i></a>';
            // $operate .= '<a  href="javascript:void(0)" class="btn action-btn btn-danger btn-xs mr-1 mb-1" title="Delete" id="delete-delivery-boys"  data-id="' . $row['id'] . '" ><i class="fa fa-trash"></i></a>';
            // $operate .= '<a href="javascript:void(0)" class=" fund_transfer btn action-btn btn-info btn-xs mr-1 mb-1" title="Fund Transfer" data-target="#fund_transfer_delivery_boy"   data-toggle="modal" data-id="' . $row['id'] . '" ><i class="fa fa-arrow-alt-circle-right"></i></a>';
            ...
            // delivery boy status
            if ($row['status'] == 0){
                $tempRow['status'] = "<label class='badge bg-warning'>Not-Approved</label>";
            } else if ($row['status'] == 1){
                $tempRow['status'] = "<label class='badge bg-success'>Approved</label>";
            }
    ...
    function get_cash_collection_list($user_id = '')
    {
        ...
        foreach ($txn_search_res as $row) {
            ...
            $tempRow['cash_received'] = $row['cash_received'];
            $tempRow['type'] = (isset($row['type']) && $row['type'] == "delivery_boy_cash") ? '<label class="badge bg-danger">Received</label>' : '<label class="badge bg-success">Collected</label>';
    ...
    public function get_delivery_boy_cash_collection($limit = "", $offset = '', $sort = 'transactions.id', $order = 'DESC', $search = NULL, $filter = "")
    {
        ...
        if (isset($search) and $search != '') {
            $multipleWhere = ['`transactions.id`' => $search, '`transactions.amount`' => $search,'`transactions.order_id`' => $search, '`transactions.date_created`' => $search, 'users.username' => $search, 'users.mobile' => $search, 'users.email' => $search, 'transactions.type' => $search, 'transactions.status' => $search];
        }
