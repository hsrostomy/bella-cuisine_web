...
class Order_model extends CI_Model
{
    ...
    public function delete_draft_orders()
    {
        ...
        foreach ($products as $order_id) {
            ...
            if ($time_diff->h >= 1 || $time_diff->days > 0) {
                ...
                // update_wallet_balance(P'credit', $user_id, $returnable_amount, 'Wallet Amount Credited for Order ID  : ' . $order['order_data'][0]['id']);
    ...
    public function place_order($data)
    {
        ...
        if (!empty($product_variant)) {
            ...
            if (defined('ALLOW_MODIFICATION') && ALLOW_MODIFICATION == 1) {
                ...
                $user_data = fetch_details('users', ['id' => $data['user_id']]);

                if (isset($system_settings['support_email']) && !empty($system_settings['support_email'])) {
                    send_mail($system_settings['support_email'], $system_settings['support_email'], $fcm_admin_subject, $fcm_admin_msg);
            ...
            // print_r($user_data[0]['email']);
            // print_r($user_data[0]['mobile']);
            notify_event(
                "place_order",
            ...
            if ($res = send_mail($system_settings['support_email'], $user_email, 'Order Confirmation - ' . $system_settings['app_name'], $this->load->view('admin/pages/view/email-template.php', $overall_order_data, TRUE))) {
                // print_r($res);
                // if (isset($res['error']) && $res['error'] == 1) {
                //     $res = send_mail($system_settings['support_email'], $user_email, 'Order Confirmation - ' . $system_settings['app_name'], 'Dear ' . $user[0]['username'] . 'We are excited to confirm your recent order with [Your Store Name]. Thank you for choosing us for your shopping needs. Your order ID is #' . $last_order_id);
                // }
            }
    ...
    {
        $res = $this->db->select('oi.*,a.name as user_name,a.mobile as recipient_contact,oi.id as order_item_id,p.*,v.product_id,o.*,o.id as order_id,o.total as order_total,o.wallet_balance,oi.active_status as oi_active_status,u.email,u.username as uname,o.status as order_status,p.name as pname,p.slug as product_slug,p.type,p.image as product_image,v.price as product_price,v.special_price as product_special_price,p.is_prices_inclusive_tax,(SELECT username FROM users db where db.id=o.delivery_boy_id ) as delivery_boy,(SELECT status FROM orders o where o.id=oi.order_id  ) as order_status ')
    ...
    public function get_orders_list(
        ...
        foreach ($user_details as $row) {
            ...
            if (!empty($row['items'])) {
                ...
                }elseif ($row['payment_method'] == 'net_banking') {
                    $payment_method = 'Net Banking';
                }elseif ($row['payment_method'] == 'bar_code') {
                    $payment_method = 'Bar Code';
