...
class Setting_model extends CI_Model
{
    ...
    public function update_system_setting($post)
    {
        ...
        $system_data = [
            ...
            'is_admin_app_under_maintenance' => (isset($post['is_admin_app_under_maintenance'])) ? '1' : '0',
            ...
            'message_for_customer_app' => (isset($post['message_for_customer_app']) ?? !empty($post['message_for_customer_app'])) ? $post['message_for_customer_app'] : '',
            'message_for_admin_app' => (isset($post['message_for_admin_app']) ?? !empty($post['message_for_admin_app'])) ? $post['message_for_admin_app'] : '',
            'message_for_web' => (isset($post['message_for_web']) ?? !empty($post['message_for_web'])) ? $post['message_for_web'] : '',
            'message_for_delivery_boy_app' => (isset($post['message_for_delivery_boy_app']) ?? !empty($post['message_for_delivery_boy_app'])) ? $post['message_for_delivery_boy_app'] : '',
            'is_offer_popup_on' => (isset($post['is_offer_popup_on']) && !empty($post['is_offer_popup_on'])) ?: 0,
            ...
            'offer_popup_method' => (isset($post['offer_popup_method']) && !empty($post['offer_popup_method'])) ? $post['offer_popup_method'] : '',
            'local_pickup' => (isset($post['local_pickup'])) ? '1' : '0',
            'address' => (isset($post['address']) && $post['address'] != '') ? $post['address'] : '',
            'latitude' => (isset($post['latitude']) && $post['latitude'] != '') ? $post['latitude'] : '',
            'longitude' => (isset($post['longitude']) && $post['longitude'] != '') ? $post['longitude'] : '',
    ...
    public function update_payment_method($post)
    {
        ...
        $payment_data['razorpay_webhook_secret_key'] = isset($post['razorpay_webhook_secret_key']) && !empty($post['razorpay_webhook_secret_key']) ? $post['razorpay_webhook_secret_key'] : '';
    ...
    public function get_time_slot_details()
    {
        ...
        foreach ($search_res as $row) {
            ...
            <a href="javascript:void(0)" class="edit_btn dropdown-item" title="Edit" data-id="' . $row['id'] . '" data-url="admin/time-slots/"><i class="fa fa-pen"></i> Edit</a>
              <a href="javascript:void(0)" class=" dropdown-item" id="delete-time-slot" data-id=' . $row['id'] . ' title="Delete" ><i class="fa fa-trash"></i> Delete</a></div>';
            ...
            $tempRow['status'] = ($row['status'] == 1) ? "<span class='badge bg-success'>Active</span>" : "<span class='badge bg-danger'>Deactive</span>";
    ...
    public function get_theme_list()
    {
        ...
        foreach ($theme as $row) {
            ...
            $tempRow['image'] = "<a href='" . base_url('assets/front_end/' . $row['slug'] . '/preview-image/' . $row['image']) . "' data-toggle='lightbox' data-gallery='gallery'><img src='" . base_url('assets/front_end/' . $row['slug'] . '/preview-image/' . 'modern.jpg') . "' class='img-fluid rounded col-md-2'></a>";
            if ($row['is_default'] == '1') {
                $tempRow['is_default'] = '<a class="badge bg-success text-white" >Yes</a>';
            } else {
                $tempRow['is_default'] = '<a class="badge bg-danger text-white" >No</a>';
            ...
            if ($row['status'] == '1') {
                ...
                $tempRow['status'] .= '<a class="form-switch update_active_status " data-table="themes" title="Deactivate" href="javascript:void(0)" data-id="' . $row['id'] . '" data-status="' . $row['status'] . '" ><input class="form-check-input " type="checkbox" role="switch" checked></a>';
            } else {
                $tempRow['status'] = '<a class="badge bg-danger text-white" ></a>';
                $tempRow['status'] .= '<a class="form-switch update_active_status mr-1 mb-1" data-table="themes" title="Deactivate" href="javascript:void(0)" data-id="' . $row['id'] . '" data-status="' . $row['status'] . '" ><input class="form-check-input " type="checkbox" role="switch" ></a>';
            }
