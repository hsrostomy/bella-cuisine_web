...
class System_users_model extends CI_Model
{
    ...
    function get_users_list()
    {
        ...
        <a href="javascript:void(0)" class="edit_btn dropdown-item"  title="Edit" data-id="' . $row['id'] . '" data-url="admin/system_users/add_system_users"><i class="fa fa-pen"> </i> Edit</a>
            <a href="javascript:void(0)" class=" dropdown-item" id="delete-system-users" data-id=' . $row['user_id']  . ' title="Delete" ><i class="fa fa-trash"></i> Delete</a></div>';


                // $operate .= ' <a href="javascript:void(0)" class="edit_btn btn action-btn btn-success btn-xs mr-1 mb-1"  title="Edit" data-id="' . $row['id'] . '" data-url="admin/system_users/add_system_users"><i class="fa fa-pen"></i></a>';
                // $operate .= ' <a  href="javascript:void(0)" class="btn action-btn btn-danger btn-xs mr-1 mb-1"  title="Delete" id="delete-system-users" data-id="' . $row['user_id'] . '"  ><i class="fa fa-trash"></i></a>';

                if ($row['active'] == '1') {
                    $tempRow['status'] = '<a class="badge bg-success text-white" ></a>';
                    $tempRow['status'] .= '<a class="form-switch update_active_status " data-table="users" title="Deactivate" href="javascript:void(0)" data-id="' . $row['user_id'] . '" data-status="' . $row['active'] . '" ><input class="form-check-input " type="checkbox" role="switch" checked></a>';
                } else {
                    $tempRow['status'] = '<a class="badge bg-danger text-white" ></a>';
                    $tempRow['status'] .= '<a class="form-switch update_active_status mr-1 mb-1" data-table="users" title="Deactivate" href="javascript:void(0)" data-id="' . $row['user_id'] . '" data-status="' . $row['active'] . '" ><input class="form-check-input " type="checkbox" role="switch" ></a>';
                }
                ...
                //     $operate .= '<a class="btn action-btn btn-warning btn-xs update_active_status mr-1" data-table="users" title="Deactivate" href="javascript:void(0)" data-id="' . $row['user_id'] . '" data-status="' . $row['active'] . '" ><i class="fa fa-eye-slash"></i></a>';
                // } else {
                //     $tempRow['active'] = '<a class="badge bg-danger text-white" >Inactive</a>';
                //     $operate .= '<a class="btn action-btn btn-primary mr-1 btn-xs update_active_status" data-table="users" href="javascript:void(0)" title="Active" data-id="' . $row['user_id'] . '" data-status="' . $row['active'] . '" ><i class="fa fa-eye"></i></a>';
                // }
            }
            ...
            if ($row['role'] == '0') {
                $row['role'] = "<span class='badge bg-primary'>Super Admin</span>";
            }
            if ($row['role'] == '1') {
                $row['role'] = "<span class='badge bg-danger'>Admin</span>";
            }
            if ($row['role'] == '2') {
                $row['role'] = "<span class='badge bg-warning'>Editor</span>";
            }
