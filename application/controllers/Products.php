...
class Products extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(['cart_model', 'category_model', 'rating_model', 'product_model', 'product_faqs_model', 'brand_model']);
    ...
    public function index()
    {
        ...
        $this->data['products'] = fetch_product($user_id, $filter, null, $category_id, $limit, $offset, $sort, $order, null, null);
    ...
    public function offers_and_flash_sale()
    {
        ...
        $this->data['main_page'] = 'offers_and_flash_sale';
        $this->data['title'] = 'Offers & Flash Sale | ' . $this->data['web_settings']['site_title'];
        $this->data['keywords'] = 'Offers & Flash Sale, ' . $this->data['web_settings']['meta_keywords'];
        $this->data['description'] = 'Offers & Flash Sale | ' . $this->data['web_settings']['meta_description'];
        $this->data['page_main_bread_crumb'] = "Offers & Flash Sale";
    ...
    public function details($slug = '')
    {
        ...
        $product = fetch_product($user_id, ['slug' => $slug], NULL, NULL, NULL, NULL, NULL, NULL, NULL, $valid_zipcode, 1);
        ...
        $this->data['related_products'] = fetch_product($user_id, ['is_similar_products' => 1], $product['product'][0]['id'], $product['product'][0]['category_id'], 12);
        ...
        if ($this->ion_auth->logged_in()) {
            ...
            $this->data['faq'] = $this->product_model->get_product_faqs('', $product['product'][0]['id'], '', '', 0, 10, 'id', 'DESC');
        }
    ...
    public function flash_sale($sale_id = '', $sale_title = '')
    {
        if (empty($sale_id)) {
            redirect(base_url());
        }
        ...
        $section = $this->db->where('id', $sale_id)->get('flash_sale')->row_array();
        if (empty($section)) {
            redirect(base_url());
        }
        ...
        $total_rows = fetch_product($user_id, $filter, $product_ids, $product_categories, null, null, null, null, TRUE);
        $config['base_url'] = base_url('products/flash_sale/' . $sale_id . '/' . $sale_title);
        $config['total_rows'] = $total_rows;
        $config['per_page'] = $limit;
        $config['uri_segment'] = 5;
        $config['use_page_numbers'] = TRUE;
        ...
        $this->data['flash_sale'] = $flash_sale;
        $this->data['filters'] = (isset($this->data['products']['filters'])) ? json_encode($this->data['products']['filters']) : "";
        $this->data['filters_key'] = 'products_section_' . $sale_id;
        $this->data['page_main_bread_crumb'] = $page_title;
        $this->load->view('front-end/' . THEME . '/template', $this->data);
    }
    ...
    public function add_faqs()
    {
        if ($this->ion_auth->logged_in()) {
            ...
            $this->response['error'] = true;
            $this->response['message'] = 'Please Login first to add FAQ.';
            echo json_encode($this->response);
            return false;
        }
    }
    ...
    public function manage_offers()
    {
        ...
        if (isset($id) && !empty($id)) {
            $offer_details = fetch_details('offers', ['id' => $id]);

            $filters['min_discount'] = (isset($offer_details[0]['min_discount'])) ? $offer_details[0]['min_discount'] : 0;
            $filters['max_discount'] = (isset($offer_details[0]['max_discount'])) ? $offer_details[0]['max_discount'] : 0;

            if (strtolower($offer_details[0]['type']) == "products") {

                $id = (!empty($offer_details[0]['type_id']) && isset($offer_details[0]['type_id'])) ? $offer_details[0]['type_id'] : '';
                $pro_res = fetch_product(NULL, NULL, $id);
                $product['product'] = $pro_res['product'];
            } else if (strtolower($offer_details[0]['type']) == "categories") {

                $cat_id = (!empty($offer_details[0]['type_id']) && isset($offer_details[0]['type_id'])) ? $offer_details[0]['type_id'] : '';
                $pro_res = fetch_product('', (isset($filters)) ? $filters : null, '', $cat_id);
                $product = $pro_res;
            } else if (strtolower($offer_details[0]['type']) == "all_products") {
                $pro_res = fetch_product('', (isset($filters)) ? $filters : null, '', '');
                $product = $pro_res;
            } else {
                $product  =  [];
            }
    ...
    public function download_link_hash()
    {
        ...
        if (isset($oreder_item_data) && !empty($oreder_item_data) && isset($transaction_data) && !empty($transaction_data)) {
            if ($order_item_id == $oreder_item_data[0]['id'] && $user_id == $oreder_item_data[0]['user_id']) {
                if ($transaction_data[0]['status'] == 'success' || $transaction_data[0]['status'] == 'received') {
    ...
    public function manage_popup_offers()
    {
        $id = $this->uri->segment('3');
        if (isset($id) && !empty($id)) {
            $offer_details = fetch_details('popup_offers', ['id' => $id]);
