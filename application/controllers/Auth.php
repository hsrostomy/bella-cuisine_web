...
class Auth extends CI_Controller
{
    ...
    public function forgot_password()
    {
        ...
        if ($this->config->item('identity', 'ion_auth') != 'email') {
            ...
            $this->form_validation->set_rules('identity', $this->lang->line('forgot_password_validation_email_label'), 'required|xss_clean|valid_email');
        }
    ...
    public function deactivate($id = NULL)
    {
        ...
        $this->form_validation->set_rules('id', $this->lang->line('deactivate_validation_user_id_label'), 'required|xss_clean|alpha_numeric');
    ...
    public function create_user()
    {
        ...
        if ($identity_column !== 'email') {
            $this->form_validation->set_rules('identity', $this->lang->line('create_user_validation_identity_label'), 'trim|required|xss_clean|is_unique[' . $tables['login_users'] . '.' . $identity_column . ']');
            $this->form_validation->set_rules('email', $this->lang->line('create_user_validation_email_label'), 'trim|required|xss_clean|valid_email');
        } else {
...
// print_r($_POST);
            if (!isset($_POST['forget_password_val']) && isset($_POST['mobile']) && is_exist(['mobile' => $_POST['mobile']], 'users')) {
                $this->response['error'] = true;
    ...
    public function register_user()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $additional_data = [
                ...
                'mobile' => (isset($_POST['mobile']) && !empty($_POST['mobile'])) ? $_POST['mobile'] : 0,
                'dob' => $this->input->post('dob'),
                'city' => $this->input->post('city'),
                'area' => $this->input->post('area'),
                'country_code' => (isset($_POST['country_code']) && !empty($_POST['country_code'])) ? str_replace('+', '', $_POST['country_code']) : 0,
