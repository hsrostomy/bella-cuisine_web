...
*/
            ...
            if (!$this->form_validation->run()) {
                ...
                foreach ($messages['msg'] as $row) {
                    ...
                    foreach ($media_files as $key => $media_file) {
                        $file_extension = explode('.', $media_file['original_file_name']);
                        $media_file['file_extension'] = end($file_extension);
                        $media_file['file_url'] = base_url('uploads/chat_media/' . $media_file['original_file_name']);
                        $message['msg'][$i]['media_files'][$key] = $media_file;
                    }
        ...
        */
            ...
            if (!$this->form_validation->run()) {
                ...
                $search_res = $this->db->select('u.id,u.username,u.email,u.image')->join('users_groups', 'users_groups.user_id = u.id')->where("users_groups.group_id", 1);
                ...
                $res = $search_res->group_by('u.id')->order_by($sort, $order)->limit($limit, $offset)->get('users u')->result_array();
