...
class Tickets extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        // unset($_SESSION['authorize_flag']); before
            ...
            if (!$this->form_validation->run()) {
                ...
                if (!$this->ticket_model->add_ticket($data)) {
                    ...
                    if (!empty($result)) {
                        ...
                        $string = isset($custom_notification[0]) ? json_encode($custom_notification[0]['message'], JSON_UNESCAPED_UNICODE) : '';
                        ...
                        if (isset($ticket_res[0]['user_id'])) {
                            $user_res = fetch_details("users", ['id' => $ticket_res[0]['user_id']], 'fcm_id', '',  '', '', '');
                        }

                        $fcm_ids[0][] = isset($user_res[0]) ? $user_res[0]['fcm_id'] : [];
