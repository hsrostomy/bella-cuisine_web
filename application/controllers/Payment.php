...
class Payment extends CI_Controller
{
    ...
    /** can be used by all other payment methods if required */
    public function delete_order_general($order_id)
    {
        if (!$this->ion_auth->logged_in()) {
            $response["error"] = true;
        ...
        $response["message"] = "Order deleted and items added back to cart!";
        return $response;
    }

    public function paypal_wallet()
    {
        ...
        //    return;
        // Set variables for paypal form
        $returnURL = base_url() . 'payment/wallet_success';
        $cancelURL = base_url() . 'payment/cancel';
        $notifyURL = base_url() . 'app/v1/api/ipn';
        ...
        if ($_POST['type'] == 'wallet') {
            ...
            if ($this->data['is_logged_in']) {
                ...
                $paytmParams["body"] = array(
                    ...
                    "callbackUrl"   => 'https://webhook.site/b011690b-192d-4765-b231-c5508ae44ec7',
                    "userInfo"      => array(
                        "custId"    => $user_id,
                    ),
                );
