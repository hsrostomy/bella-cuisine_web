...
class MyConfig
{
    ...
    function loadSystemResources()
    {
        if (!method_exists('MyConfig', 'verify_doctor_brown')) {
            ...
            $admin_app_doctor_brown = get_settings('admin_app_doctor_brown', true);
    ...
    function verify_doctor_brown()
    {
        ...
        if ((empty($doctor_brown) && empty($web_doctor_brown) && empty($admin_app_doctor_brown)) && !in_array(current_url(), $exclude_uris)) {
            ...
            if ((!empty($admin_app_doctor_brown) && !in_array(current_url(), $exclude_uris))) {
                /* redirect him to the page where he can enter the purchase code */

                $calculated_time_check = $time_check = '';

                $time_check = (isset($admin_app_doctor_brown["time_check"])) ? trim($admin_app_doctor_brown["time_check"]) : "";
                $code_bravo = (isset($admin_app_doctor_brown["code_bravo"])) ? trim($admin_app_doctor_brown["code_bravo"]) : "";
                $code_adam = (isset($admin_app_doctor_brown["code_adam"])) ? trim($admin_app_doctor_brown["code_adam"]) : "";
                $dr_firestone = (isset($admin_app_doctor_brown["dr_firestone"])) ? trim($admin_app_doctor_brown["dr_firestone"]) : "";
                $str = $code_bravo . "|" . $code_adam . "|" . $dr_firestone;
                $calculated_time_check = hash('sha256', $str);
                if (empty($calculated_time_check) || empty($time_check)) {
                    if (!in_array(current_url(), $exclude_uris)) {
