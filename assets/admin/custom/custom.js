...
var btn = $('#web-form-rest');
console.log(btn);
console.log('test');
$('#web-form-rest').on('click', function (e) {
    console.log(e);
    $('#system_setting_form')[0].reset();
});
    ...
    swal
        ...
        .then(willDelete => {
            if (willDelete) {
                $.ajax({
                    url: base_url +
                        'admin/Notification_settings/mark_all_as_read/' +
                        user_id,
                    type: 'POST',
    ...
    setInterval(function () {
        $.ajax({
            ...
            success: function (result) {
                var html = ''
                html +=
                    '<a href="" id="notification_count" data-toggle="dropdown" class="nav-link notification-toggle nav-link-lg"><i class="fas fa-bell"></i><span class="badge bg-danger navbar-badge order_notification">' +
                    result.count_notifications +
                    '</span></a>'
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(result.notifications, function (i, a) {
                beep =
                    a.read_by && a.read_by == 0 ?
                        '<span><i class="fa fa-certificate ml-3 text-orange text-sm"></i></span>' :
                        ''
            ...
            html +=
                '<a href="' +
                base_url +
                'admin/Notification_settings/manage_ststem_notifications' +
...
$(document).ready(function () {
    ...
    tinymce.init({
        ...
        toolbar: 'undo redo | image media | code fullscreen| formatpainter casechange blocks | bold italic forecolor backcolor | ' +
            'alignleft aligncenter alignright alignjustify | ' +
            'bullist numlist checklist outdent indent | removeformat | ltr rtl |a11ycheck code table help',
    ...
    $('.swatche_type').each(function () {
        $('.swatche_type').select2({
            ...
            width: $('.swatche_type').data('width') ?
                $('.swatche_type').data('width') : $('.swatche_type').hasClass('w-100') ?
                    '100%' : 'style',
