...
$(document).ready(function () {
    ...
    function updatePagination() {
        ...
        const pageButtons = document.querySelectorAll('.page-item:not(:first-child):not(:last-child) a');
...
$(document).ready(function () {
    function toggleDiscountFields() {
        ...
        if (offerType === "default" || offerType === "products" || offerType === "offer_url") {
...
$('#attribute_modal_close').on('click', function () {
    console.log("hee in click attribute close");
                    ...
                    (n = function (e, t, a) {
                        ...
                        n.on('created', function (t) {
                            ...
                            'line' === t.type ?
                                ...
                                'point' === t.type &&
                                new Chartist.Svg(t.element._node.parentNode).elem('line', {
                                    x1: t.x,
                                    y1: t.y,
                                    x2: t.x + 0.01,
                                    y2: t.y,
                                ...
                                t.element.animate({
                                    d: {
                                        ...
                                        from: t.path
                                            .clone()
                                            .scale(1, 0)
                                            .translate(0, t.chartRect.height())
                                            .stringify(),
                        ...
                        (c = function (e) {
                            switch (
                            (
                                e || a('#ecommerceChartView .chart-action').find('.active')
                            ).attr('href')
                            ) {
                                case '#scoreLineToDay':
        ...
        success: function (data) {
            ...
            $.each(result, function (a, b) {
                ...
                if (
                    ...
                    html +=
                        '<div class="form-group row mt-4"><div class="col col-xs-12"><label class="control-label">Price :</label><input type="number" name="variant_price[]" class="col form-control price varaint-must-fill-field" min="1" step="0.01"></div><div class="col col-xs-12"><label class="control-label">Special Price :</label><input type="number" name="variant_special_price[]" class="col form-control discounted_price" min="0" step="0.01"></div></div>' +
                        '<div class="form-group row mt-4">' +
                        '<div class="col col-xs-12">' +
                        '<label for="weight" class="control-label col-md-12">Weight <small>(kg)</small> <span class="text-danger text-xs">*</span></label>' +
                        '<input type="number" min="0" class="form-control" name="weight[]" placeholder="Weight" id="weight" value="" step="0.01">' +
            ...
            success: function (result) {
                ...
                if (!$.isEmptyObject(result)) {
                    $.each(result, function (index, value) {
                        html +=
                            '<li class="list-group-item d-flex bg-gray-light align-items-center h-25 ui-sortable-handle" id="product_id-' +
                            value['id'] +
                            '">'
                        ...
                        html +=
                            ...
                            ' </span></div>'
                        html +=
                            '<div class="col-md-4"><span>' + value['name'] + '</span></div>'
                        html +=
                    ...
                    html +=
                        '<li class="list-group-item d-flex justify-content-center bg-gray-light align-items-center h-25 ui-sortable-handle" id="product_id-3"><div class="col-md-12 d-flex justify-content-center"><span>No Products  Are  Available</span></div></li>'
                    $('#sortable').html(html)
                }
            }
        })
        ...
        $.each(data, function (a, b) {
            ...
            if (
                ...
                html +=
                    ...
                    b.price +
                    '" min="1" step="0.01"></div><div class="col col-xs-12"><label class="control-label">Special Price :</label><input type="number" name="variant_special_price[]" class="col form-control discounted_price" min="0" value="' +
                    b.special_price +
...
//form-submit-event
$(document).on('submit', '.container-fluid .form-submit-event,.form-submit-event', function (e) {
    e.preventDefault()
...
$(document).on('click', '.update_active_status', function () {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result['error'] == true) {
                iziToast.success({
                    message: '<span style="text-transform:capitalize">' +
                        result.message +
                        '</span> Status Updated'
                })
                $('.table').bootstrapTable('refresh')
...
// if (typeof resetForm !== 'undefined' && resetForm) {
//     console.log("here");
//     document.getElementById("add_product_form").reset();
// }
...
$(document).on('click', '.edit_order_tracking', function (e, rows) {
    console.log('here in edit_order_tracking function');
    e.preventDefault()
...
function priceFormatter(data) {
    ...
    return (
        '<span style="color:green;font-weight:bold;font-size:large;">' +
        store_currency +
        data
            .map(function (row) {
                return +row[field]
...
$(document).on('click', '#notification_count', function (e, rows) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            var seconds_ago
            var time
            var url = base_url + 'admin/flash_sale';


            $.each(result.notifications, function (i, a) {
                beep =
                    a.read_by && a.read_by == 0 ?
                        '<span><i class="fa fa-certificate mx-3 text-warning text-sm"></i></span>' :
                        ''
                seconds_ago = a.date_sent

                if (a.type == 'place_order') {
                    url = base_url + 'admin/orders/edit_orders' +
                        '?edit_id=' +
                        a.type_id +
                        '&noti_id=' +
                        a.id
                ...
                html +=
                    '  <a href="' + url + '" class="list - group list - group - flush pt - 3 px - 3">\
                            <div class="d - flex">\
                    ...
                    a.message.replace(/\\'/g, "'") +
                    '</p>\
                                    <p class="text-sm text-muted"><i class="far fa-clock"></i><span class="mx-1">' +
                    seconds_ago +
                    '</span></p>\
                                </div>\
                            </div>\
            ...
            if (!(result.notifications.length === 0)) {
                html += '<a href="javascript:void(0);" class="dropdown-item dropdown-footer mark-all-notifications-as-read">Mark All As Read</a>';
            } else {
                html += '<div class="dropdown-footer p-3">No New Notifications</div>';
            }

            html += '<div class="dropdown-divider"></div><a href="' + base_url + 'admin/Notification_settings/manage_ststem_notifications' + '" class="dropdown-item dropdown-footer">See All Notifications</a>';
...
function formatOffers(offer) {
    ...
    <div class="col-md-1 align-self-center">
        ...
        <div class="align-self-center col-md-10">
            <div class="">Min - Max Discount : ${offer.min_discount}% - ${offer.max_discount}% 
        </div>
...
$(document).on('switchChange.bootstrapSwitch', '#is_specific_user', function (
    event
) {
    event.preventDefault()
    var state = $(this).bootstrapSwitch('state')
