...
$(document).on('click', '.add_to_cart_pos', function (e) {
  ...
  // console.log(shopItem.children[3]);
  console.log(shopItem.children[3].children[0]);
  var variant_dropdown = shopItem.children[3].children[0]
...
$(document).on('click', '.cart-quantity-input', function (e) {
  ...
  input = operation == 'minus' ? qty - 1 : qty + 1
...
function display_products(products) {
  ...
  for (i = 0; i < products.length; i++) {
    ...
    display_products +=
      ...
      base_url +
      'admin/product/view-product?edit_id=' +
      products[i].id +
      ' target="_BLANK"><b class="text text-dark">' +
      products[i].name +
      '</b><div class="title-overlay"></div></a>' +
      ' </span>' +
      '<div class="shop-item-image d-flex align-items-center justify-content-center">' +
      '  <img class="item-image" src="' +
      products[i].image +
      '">' +
      '</div>' +
      '<div class="custom-dropdown mt-4">' +
      '<select class="form-control product-variants variant_value" id="change">'
    ...
    display_products +=
      '</select>' +
      '<span class="dropdown-caret"></span>' + // Add dropdown caret
      '</div>';

    // Check if stock is not null, then show Add to Cart button
    if ((((products[i].stock_type).length != 0) && (products[i].stock).length != 0)) {
      display_products +=
        '<div class="shop-item-details justify-content-center">' +
        ' <button class="btn btn-xs btn-info shop-item-button p-2 add_to_cart_pos"  type="button">Add to Cart</button>' +
        ' </div>';
    }
