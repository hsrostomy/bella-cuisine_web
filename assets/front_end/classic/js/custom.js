...
if (auth_settings == "firebase") {
    ...
    window.onload = function () {
        ...
        document.getElementById("offer_popup_value").value;
        if (document.getElementById("offer_popup_value").value == "refresh") {
            $("#offer_popup").modal("show");
        } else if (
            document.getElementById("offer_popup_value").value == "session_storage"
        ) {
            $("#offer_popup").modal("show");
        }
    }
...
$('#leftside-navigation .sub-menu > a').click(function (e) {
    ...
    !$('#leftside-navigation .sub-menu > a').next().is(':visible') ?
        $('#leftside-navigation .sub-menu > a')
            .find('.arrow')
            .removeClass('fa-angle-down')
            .addClass('fa-angle-left') :
        ''
    $(this).find('.arrow').hasClass('fa-angle-left') ?
        $(this)
            .find('.arrow')
...
$(function () {
    ...
    function load_review_images(
        product_id,
        limit,
        offset,
        target = '#user_image_data'
    ) {
        $('#review-image-title').data('review-offset', offset + limit)
        $.getJSON(
            base_url +
            'products/get_rating?product_id=' +
            product_id +
            '&has_images=1&limit=' +
            limit +
            '&offset=' +
                ...
                if (data.error == false) {
                    for (var i = 0; i < data.data.product_rating.length; i++) {
                        ...
                        for (var j = 0; j < obj.images.length; j++) {
                            var obj3 = obj.images
                            image_data +=
                                "<div class='review-box '><a href='" +
                                obj3[j] +
                                "' data-lightbox='review-images-12345' data-title='<font >" +
                                obj.rating +
                                ' &#9733;</font></br>' +
                                obj.user_name +
                                '<br>' +
        ...
        $('#quick-view').iziModal({
            ...
            onOpening: function (modal) {
                ...
                $.getJSON(
                    ...
                    modal.$element.data('dataProductId'),
                    function (data) {
                        var total_images = 0
                        ...
                        let brand = '<a href="' + base_url + "products?brand=" + data.brand_slug + '">' + data.brand + '</a>'
                        ...
                        $.each(data.variant_attributes, function (i, e) {
                            ...
                            variant_attributes +=
                                '<h4>' +
                                e.attr_name +
                                '</h4><div class="btn-group btn-group-toggle" data-toggle="buttons">'
                            $.each(attribute_ids, function (j, id) {
                                ...
                                if (swatche_types[j] == '1') {
                                    ...
                                    variant_attributes +=
                                        ...
                                        '" value="' +
                                        id +
                                        '" class="modal-product-attributes" autocomplete="off"><br>' +
                                        '</button>'
                                } else if (swatche_types[j] == '2') {
                                    is_image = 1
                                    variant_attributes +=
                                        '<style> .product-page-details .btn-group>.active { color: #000000; border: 1px solid black;}</style>' +
                                        '<label class="btn text-center bg-transparent">' +
                                    ...
                                    variant_attributes +=
                                        ...
                                        '" value="' +
                                        id +
                                        '" class="modal-product-attributes" autocomplete="off">' +
                                        attribute_values[j] +
                                        '<br>' +
                                        '</label>'
                                }
                        ...
                        var err_msg =
                            data.zipcode != '' && typeof data.zipcode !== 'undefined' ?
                                '<b class="text-' +
                                className +
                                '">Product is ' +
                                is_not +
                        ...
                        variant_attributes +=
                            ...
                            csrfHash +
                            '">' +
                            '<input type="text" class="form-control" id="zipcode" placeholder="Zipcode" name="zipcode" required value="' +
                            data.zipcode +
                            '">' +
                            '</div>' +
                            '<button type="submit" class="button button-primary-outline m-0" data-product_id="' +
                            data.id +
                            '"  data-zipcode="' +
                            data.zipcode +
                            '"  id="validate_zipcode">Check Availability</button>' +
                            '</div>' +
                        ...
                        $.each(data, function (i, e) {
                            compare +=
                                '<button type="button" name="compare" class="buttons btn-6-6 extra-small m-0 compare" id="compare" data-product-id="' +
                                data.id +
                                '" data-product-variant-id="' +
                                data.variants.id +
                                '"><i class="fa fa-random"></i> Compare</button>'
                        })
                        ...
                        if (!$.isEmptyObject(data.tags)) {
                            ...
                            $.each(data.tags, function (i, e) {
                                tags +=
                                    ...
                                    'products/tags/' +
                                    e +
                                    '" target="_blank"><span class="badge bg-secondary p-1 mr-1">' +
                                    e +
                                    '</span></a>'
                            })
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.error == false) {
                    ...
                    //   var variant =
                    //     typeof e.product_variants.variant_values != 'undefined' &&
                    //     e.product_variants.variant_values != null
                    //       ? e.product_variants.variant_values
                    //       : ''
                    //   var price =
                    //     e.special_price < e.price && e.special_price != 0
                    //       ? e.special_price
                    //       : e.price
                    ...
                    //     '<div class="product-price align-self-center">' +
                    //     currency +
                    //     ' ' +
                    //     price +
                    //     '</div>' +
                    ...
                    //     '<button class="remove-product button button-danger" data-id="' +
                    //     e.product_variant_id +
                    //     '">' +
                    //     '<i class="fa fa-trash"></i>' +
                    //     '</button>' +
                    //     '</div>' +
                    //     '<div class="product-line-price align-self-center px-1">' +
                    //     currency +
                    //     ' ' +
...
$(document).ready(function () {
    ...
    $(document).on(
        'click',
        '.product-removal button,.product-removal i,.product-sm-removal button',
        function (e) {
            e.preventDefault()
            var id = $(this).data('id')
...
jQuery(document).ready(function ($) {
    ...
    morphDropdown.prototype.showDropdown = function (item) {
        ...
        if (this.mq == 'desktop') {
            ...
            var width =
                selectedDropdown.children('.content').children('ul').children('li')
                    .length * 180
            if (width > 540) {
                width = 540
            }
...
$('.navbar-top-search-box input').on('blur', function () {
    $('.navbar-top-search-box .input-group-text').css(
        'border',
        '1px solid #ced4da'
    )
})
...
$(document).ready(function () {
    $('.open-navbar-mobile').on('click', function (event) {
        event.preventDefault();
        $('.block-div').css('width', '100%')
        $('body').css('overflow', 'hidden')
        $('#mySidenav').removeClass('is-closed-left')
...
$(document).ready(function () {
    $('.shopping-cart-sidebar-btn').on('click', function (event) {
        event.preventDefault();
        $('.block-div').css('width', '100%');
        $('body').css('overflow', 'hidden');
        $('.shopping-cart-sidebar').removeClass('is-closed-right');
...
$(document).ready(function () {
    $('.cart-sidebar-div').on('click', function () {
        $('.block-div').css('width', '0%');
        $('body').css('overflow', 'unset');
        $('.shopping-cart-sidebar').addClass('is-closed-right');
        $('#mySidenav').addClass('is-closed-left');
...
jQuery(document).ready(function ($) {
    $('ul.color-style .blue').click(function () {
        $('#color-switcher').attr(
            'href',
            base_url + 'assets/front_end/classic/css/colors/blue.css'
        )
        return false
    })
    ...
    $('ul.color-style .cyan-dark').click(function () {
        $('#color-switcher').attr(
            'href',
            base_url + 'assets/front_end/classic/css/colors/cyan-dark.css'
        )
        return false
    })
    ...
    $('ul.color-style .dark-blue').click(function () {
        $('#color-switcher').attr(
            'href',
            base_url + 'assets/front_end/classic/css/colors/dark-blue.css'
        )
        return false
    })
    ...
    $('ul.color-style .dark-purple').click(function () {
        $('#color-switcher').attr(
            'href',
            base_url + 'assets/front_end/classic/css/colors/dark-purple.css'
        )
        return false
    })
    ...
    $('ul.color-style .default').click(function () {
        $('#color-switcher').attr(
            'href',
            base_url + 'assets/front_end/classic/css/colors/default.css'
        )
        return false
    })
    ...
    $('ul.color-style .green').click(function () {
        $('#color-switcher').attr(
            'href',
            base_url + 'assets/front_end/classic/css/colors/green.css'
        )
        return false
    })
    ...
    $('ul.color-style .indigo').click(function () {
        $('#color-switcher').attr(
            'href',
            base_url + 'assets/front_end/classic/css/colors/indigo.css'
        )
        return false
    })
    ...
    $('ul.color-style .orange').click(function () {
        $('#color-switcher').attr(
            'href',
            base_url + 'assets/front_end/classic/css/colors/orange.css'
        )
        return false
    })
    ...
    $('ul.color-style .peach').click(function () {
        $('#color-switcher').attr(
            'href',
            base_url + 'assets/front_end/classic/css/colors/peach.css'
        )
        return false
    })
    ...
    $('ul.color-style .pink').click(function () {
        $('#color-switcher').attr(
            'href',
            base_url + 'assets/front_end/classic/css/colors/pink.css'
        )
        return false
    })
    $('ul.color-style .purple').click(function () {
        $('#color-switcher').attr(
            'href',
            base_url + 'assets/front_end/classic/css/colors/purple.css'
        )
        return false
    })
    $('ul.color-style .red').click(function () {
        $('#color-switcher').attr(
            'href',
            base_url + 'assets/front_end/classic/css/colors/red.css'
        )
        return false
    })
...
$('#blog_search').on('change', function (e) {
    e.preventDefault();
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.error == false) {
                    ...
                    //     currency +
                    //     ' ' +
                    //     (min * price).toLocaleString(undefined, {
                    //       minimumFractionDigits: 2
                    //     }) +
                    //     '</div>' +
                    //     '</div>' +
                ...
                if (compare !== null && compare_count > 0) {
                    ...
                    $.each(result.data.product, function (i, e) {
                        ...
                        var data_max = e.total_allowed_quantity ?
                            ...
                            1
                        comp +=
                            '<td class="compare_item text-center text-justify">' +
                            '<div class="text-right"><a class="remove-compare-item"' +
                            'data-product-id="' +
                            e.id +
                            '" style="padding: 4px 8px border:0px !important" >' +
                            '<i class="fa-times fa-times-plus fa-lg fa link-color"></i></a></div><br>' +
                            '<div class="product-grid" style="border:1px !important; padding:0 0 0px;"><div class="product-image"><div class="product-image-container"><a href="products/details/' +
                            e.slug +
                            '"><img class="pic-1" src="' +
                            e.image +
                            '"></a></div></div><div itemscope itemtype="https://schema.org/Product">'
                        if (e.rating && e.no_of_rating != '') {
                            comp +=
                                '<div class="col-md-12 mb-3 product-rating-small" dir="ltr" itemprop="aggregateRating" itemscope itemtype="https://schema.org/AggregateRating"><meta itemprop="reviewCount" content="' +
                                e.no_of_rating +
                                '" /><meta itemprop="ratingValue" content="' +
                                e.rating +
                                '" /><input type="text" class="kv-fa rating-loading" value="' +
                                e.rating +
                                '" data-size="sm" title="" readonly> <span class="my-auto mx-3"> ( ' +
                                e.no_of_ratings +
                                ' reviews) </span></div>'
                        } else {
                            comp +=
                                '<div class="col-md-12 mb-3 product-rating-small" dir="ltr"><input type="text" class="kv-fa rating-loading" value="' +
                                e.rating +
                                '" data-size="sm" title="" readonly> <span class="my-auto mx-3"> ( ' +
                                e.no_of_ratings +
                                ' reviews) </span></div>'
                        ...
                        comp += '</div>'
                        comp +=
                            ' <h3 class="data-product-title" ><a href="products/details/' +
                            e.slug +
                            '">' +
                            shortDescriptionWordLimit(e.name) +
                            '</a></h3>   <div class="price mb-1">' +
                            currency +
                            ' ' +
                        ...
                        comp +=
                            '  <a href="#" class="add-to-cart add_to_cart" data-product-id="' +
                            e.id +
                            '" data-product-variant-id="' +
                            variant_id +
                            '" data-izimodal-open="' +
                    ...
                    $.each(result.data.product, function (i, e) {
                        comp +=
                            '<td class="text-center text-justify" data-title="Availability">' +
                            (e.short_description ?
                                e.short_description :
                                (e.short_description = '-')) +
                            '</td>'
                    ...
                    $.each(result.data.product, function (i, e) {
                        ...
                        if (e.type == 'variable_product') {
                            comp +=
                                '<td class="text-center text-justify" data-title="variants">'
                            ...
                            comp +=
                                '<td class="text-center text-justify" data-title="variants">-</td>'
                        }
                    })
                    comp += '</tr>'
                    comp +=
                    ...
                    $.each(result.data.product, function (i, e) {
                        comp +=
                            '<td class="text-center text-justify" data-title="made in">' +
                            (e.made_in ? e.made_in : '-') +
                            '</td>'
                    })
                    comp += '</tr>'
                    ...
                    $.each(result.data.product, function (i, e) {
                        comp +=
                            '<td class="text-center text-justify" data-title="warranty period">' +
                    ...
                    $.each(result.data.product, function (i, e) {
                        comp +=
                            '<td class="text-center text-justify" data-title="Returnable">' +
                            (e.is_returnable == '1' ?
                                (e.is_returnable = 'Yes') :
                                (e.is_returnable = 'No')) +
                            '</td>'
                    ...
                    $.each(result.data.product, function (i, e) {
                        comp +=
                            '<td class="text-center text-justify" data-title="cancelable">' +
                            (e.is_cancelable == '1' ?
                                (e.is_cancelable = 'Yes') :
                                (e.is_cancelable = 'No')) +
                            '</td>'
...
$(document).on('click', '.compare-removal button', function (e) {
    ...
    if (confirm('Are you sure want to remove this?')) {
        ...
        var compare = localStorage.getItem('compare')
        compare =
            localStorage.getItem('compare') !== null ? JSON.parse(compare) : null
        if (compare) {
            var new_compare = compare.filter(function (item) {
                return item.id != id
            })
            localStorage.setItem('compare', JSON.stringify(new_compare))
            if (compare) display_compare(new_compare)
        }
...
$(document).on('submit', '#send_forgot_password_otp_form', function (e) {
    ...
    $('#forgot_password_send_otp_btn')
        ...
        .attr('disabled', true)
    var phoneNumber =
        $('.selected-dial-code').html() + $('#forgot_password_number').val()
            ...
            if (result.error == false) {
                $.each(result.data.product_rating, function (i, e) {
                    html +=
                        ...
                        '<img src="' +
                        base_url +
                        'assets/front_end/modern/images/user.png" alt="" width="65" height="65">' +
                        '</div>' +
                        '<div class="review-comment">' +
                        '<div class="rating-list">' +
                        '<div class="product-rating">' +
                        '<input type="text" class="kv-fa" value="' +
                        e.rating +
                        '" data-size="xs" title="" readonly>' +
                        '</div>' +
                        '</div>' +
    ...
    if (filters != '') {
        $.each(JSON.parse(filters), function (i, e) {
            ...
            html +=
                ...
                '<div class="card-header-custom" id="h' +
                i +
                '">' +
                '<h2 class="clearfix mb-0">' +
                '<a class="collapse-arrow btn btn-link collapsed" data-toggle="collapse" data-target="#' +
                prefix +
                i +
            ...
            $.each(attribute_values, function (j, v) {
                ...
                html +=
                    ...
                    '" value="' +
                    v +
                    '" class="toggle-input product_attributes" id="' +
                    prefix +
                    new_attr_val +
                    '" data-attribute="' +
                    e_name +
                    ...
                    attr_checked_status +
                    '>' +
                    '<label class="toggle checkbox" for="' +
                    prefix +
                    new_attr_val +
                    '">' +
...
$(document).ready(function () {
    $(document).on('click', '.update_question', function () {
        ...
        $.ajax({
            ...
            dataType: 'json',
            url: base_url + 'Tickets/update_ticket',
            success: function (result) {
...
//   $(document).on('click', '.send_message_new', function (e) {
// console.log('herewe')
//   })
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.error == false) {
                    ...
                    li.setAttribute('class', 'repaly')
                    if (image) {

                        li.innerHTML = `<p><img class="chat-img" src="${image}" alt=""></p></br><span class="time price">${time}</span>`
                        document.getElementsByName('attachments[]')[0].value = '';

                    } else {
                        li.innerHTML = `<p>${message}</p></br><span class="time price">${time}</span>`
                    }
...
function copyText() {
    ...
    setTimeout(function () {
        copyButton.innerText = "Tap to copy";
    }, 1000);
}
