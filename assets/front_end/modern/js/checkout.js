...
$(document).ready(function () {
    ...
    $('.address-modal').on('click', '.submit', function (event) {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                $('#checkout_form > .row').show({
                    message: '<h2>Please wait... Checking serviceability in your area</h2>',
                    css: {
                        border: 'none',
                        padding: '16px'
                    }
            ...
            success: function (result) {
                 console.log(result);
                 console.log(result.availability_data);
        ...
        //       if (product.is_deliverable == false) {
        //         $('#p_' + product.product_id).html(
        //           "<b class='text-danger'> "+ (product.message ?? "Not deliverable" ) +"</b>"
        //         )
            ...
            success: function (data) {
                ...
                if (data.promo_codes.length != 0) {
                    $.each(data.promo_codes, function (i, e) {
                        html +=
                            '<a class="btn col-12" id="redeem_promocode" data-value="' + e.promo_code + '" href="#" checked>' +
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (shipping_method == 1) {
                    var final_total =
                        number_format(parseFloat(sub_total) + parseFloat(delivery_charge_with_cod), 2)
                } else {
                    var final_total = parseFloat(sub_total) + parseFloat(delivery_charge)
                }
                $('#amount').val(final_total)
    ...
    $(document).on('click', '#redeem_promocode', function () {
        ...
        // $(this).find('.promocode-section').addClass('bg-body-secondary');
        $(this).addClass('bg-body-tertiary').siblings().removeClass('bg-body-tertiary');
    })
